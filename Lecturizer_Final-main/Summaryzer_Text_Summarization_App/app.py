...
@app.route('/youtube1')
def youtube1():
	return render_template('youtube1.html') 
...
def stars():
    ...
    if request.method == 'POST':
        ...
        query = f"INSERT into review_system(name,stars,comments) VALUES('{name}', {rating}, '{comments}')"
        cursor.execute(query)
        mysql.connection.commit()
        return redirect('/stars')

    query2 = "select name,stars,comments,created_at from review_system order by created_at DESC limit 5" 
    cursor.execute(query2)
    data = cursor.fetchall()
    cursor.close()
    return render_template('stars.html',review_system = data)
